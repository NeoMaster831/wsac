...
public static partial class PipeStreamExtension
{
    ...
    private static void Read(PipeStream stream, Span<byte> buffer, CancellationToken ct)
    {
    ...
    public static void Write<T>(this PipeStream stream, T value) where T : unmanaged
    {
        Span<byte> buffer = stackalloc byte[Marshal.SizeOf<T>()];
